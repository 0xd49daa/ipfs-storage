...
Browser-first TypeScript library for IPFS-based encrypted batch storage. Stateless module — client
stores manifests locally.

**Package:** `@0xd49daa/ipfs-storage` **Runtime:** Browser (primary), Deno (development/testing)
**Serialization:** Protocol Buffers **Crypto:** WebCrypto AES-GCM and `@noble/hashes`
...
**Chunk ID:** `base58(crypto.randomUUID())` → 22 chars **Chunk path:** `{id[0:2]}/{id[2:4]}/{id}` →
...
The package is symmetric-only. Callers provide a 32-byte `manifestKey` and a 16-byte `batch_id`.
Manifest records use the `manifestKey` directly. Chunk file keys are derived from the `manifestKey`
and the file path hash.

## Cryptography Mapping

Crypto uses WebCrypto AES-GCM for Vault AEAD records plus `@noble/hashes` for content hashing:
